...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const { data: profileData } = await supabase
    .from("usernames")
    .select("user_id, screen_name, avatar_url, biography, email")
    .eq("user_id", userId)
    .single()
  ...
  const description = profileData.biography
    ? profileData.biography.slice(0, 160)
    : `${screenName}さんのプロフィール`

  return {
    title: `${screenName} (@${userId})`,
    description,
    openGraph: {
      title: `${screenName} (@${userId})`,
      description,
